...
function Outpass() {
  ...
  const getOutpass = useCallback(async () => {
    const res = await fetch('/.netlify/functions/app/slc/outpasses/' + id, {
      method: 'GET',
      headers: {
  ...
  const approveHandler = async () => {
    const res = await fetch(
      '/.netlify/functions/app/slc/outpasses/' + id + '/approve',
      {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
    ...
    if (res.status === 200) {
      ...
      navigate('/slc/outpass-history')
    }
  }

  const rejectHandler = async () => {
    const res = await fetch(
      '/.netlify/functions/app/slc/outpasses/' + id + '/reject',
      {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  return (
    <div className='mt-10'>
      ...
      <div className=' flex items-center justify-center mb-10'>
        {outpass.studentLifeCoordinator.approved ? (
        ...
        {outpass.studentLifeCoordinator.rejected ? (
          <div className='border-2 bg-ezpass px-4 py-2 rounded-md font-ubuntu text-white transition-all duration-200 ease-in-out active:scale-95 tracking-wide font-semibold mr-2'>
            Rejected
          </div>
        ) : (
