...
function Outpass() {
  ...
  const getOutpass = useCallback(async () => {
    const res = await fetch('/.netlify/functions/app/warden/outpasses/' + id, {
      method: 'GET',
      headers: {
  ...
  const approveHandler = async () => {
    const res = await fetch(
      '/.netlify/functions/app/warden/outpasses/' + id + '/approve',
      {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
    ...
    if (res.status === 200) {
      ...
      navigate('/warden/outpass-history')
    }
  }

  const rejectHandler = async () => {
    const res = await fetch(
      '/.netlify/functions/app/warden/outpasses/' + id + '/reject',
      {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
