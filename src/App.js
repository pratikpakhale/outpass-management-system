...
function App() {
  const { isAuthenticated, setRole, setToken, login } = useContext(authContext)

  const LStoken = localStorage.getItem('token')
  const LSrole = localStorage.getItem('role')

  useEffect(() => {
    if (LStoken && !isAuthenticated && LSrole) {
      setToken(LStoken)
      setRole(LSrole)
      login()
    }
  }, [LStoken, isAuthenticated, setToken, setRole, login, LSrole])
      ...
      <Routes>
        ...
        {isAuthenticated && (
          <>
            ...
            <Route
              path='/slc/outpass-history'
              element={<SLCOutpassHistory />}
            />
            <Route
            ...
            <Route
              path='/slc/manage/students'
              element={<SLCManageStudents />}
            />
            <Route
